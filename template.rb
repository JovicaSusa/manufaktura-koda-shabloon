...
@install_ruby_llm   = yes?("Install ruby-llm for AI/LLM integration? [y/n]")
...
after_bundle do
  ...
  if @install_ruby_llm
    ...
    key_lines = []
    key_lines << "  config.openai_api_key    = ENV.fetch(\"OPENAI_API_KEY\", nil)" if provider_list.include?("openai")
