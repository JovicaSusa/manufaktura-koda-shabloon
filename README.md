...
|--------|---------|-------|
| Install ruby-llm? | — | Optional; adds AI/LLM integration (see below) |
| Generate ruby-llm Chat UI? | — | Only asked if ruby-llm is installed |
| Which AI providers? | `anthropic` | Comma-separated: `openai`, `anthropic`, `gemini` |
...
If you opt in during setup, [ruby_llm](https://rubyllm.com) is installed (pinned to `~> 1`):

- **Chat + Message models** — ActiveRecord-backed persistence for conversations, created by `ruby_llm:install`
- **Optional Chat UI** — controllers, views, and Turbo streaming at `/chats`, generated by `ruby_llm:chat_ui`
- **Provider initializer** — `config/initializers/ruby_llm.rb` pre-configured with `ENV.fetch`-based keys for the providers you select (openai, anthropic, gemini). Safe to leave blank until you have API keys — the app boots and all other features work without them.
